...
const CanvasMap = () => {
    ...
    const query = new URLSearchParams(location.search).get('query'); // URL에서 query 값을 추출
    ...
    // 지역 데이터를 가져오는 함수
    const getRegionData = (query) => {
        // query가 상위 지역인 경우
        if (regions[query]) {
            return regions[query];
        }

        // query가 하위 지역인 경우, 해당 상위 지역을 찾아서 데이터 반환
        for (const [province, data] of Object.entries(regions)) {
            if (data.regionData[query]) {
                return data;
            }
        }

        // 기본값으로 강원도 데이터 반환
        return regions['강원도'] || regions['경기도'];
    };

    const currentRegionData = getRegionData(query);
    console.log('Current Region Data:', currentRegionData); // 확인용 로그
    ...
    useEffect(() => {
        ...
        console.log('Query:', query); // URL에서 가져온 query 값
        console.log('Region Data:', regions[query]); // query로 가져온 데이터 확인
