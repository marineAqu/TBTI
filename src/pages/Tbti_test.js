...
function TBTITest() {
    ...
    const handleAnswer = async (value) => {
        // 답변을 비동기적으로 처리하기 전에 먼저 상태 업데이트
        const updatedAnswers = { ...answers, [currentStep]: value }; // 현재 단계의 답변 저장

        // 마지막 질문인지 확인
        if (currentStep === questions.length - 1) {
            console.log("All questions answered!");
            const result = await calculateResult(updatedAnswers); // 비동기 결과 계산
            console.log("셋:", result); // 비동기 결과 출력
            setResultData(result); // 결과 상태 업데이트
        } else {
